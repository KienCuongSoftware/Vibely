...
function statusLabel(status) {
  return STATUS_OPTIONS.find((item) => item.value === String(status ?? '').toUpperCase())?.label ?? 'Không rõ'
}
      ...
      <div className="w-full max-w-lg rounded-2xl border border-zinc-800 bg-zinc-950 p-5 shadow-2xl shadow-black/60">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-zinc-500">Bài đăng sẽ bị gỡ khỏi hệ thống và không hiển thị công khai.</p>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="mt-5 rounded-xl border border-red-500/30 bg-red-500/10 p-4 text-sm text-red-100">
          ...
          <p className="mt-2 text-red-200/90">
            Tác giả: @{post?.authorUsername || 'unknown'} • {post?.authorEmail || 'Không có email'}
          </p>
        </div>
    ...
    >
      {!authReady || loading ? (
        ...
        <section className="rounded-xl border border-zinc-800 bg-zinc-900/70 px-4 py-16 text-center">
          ...
          <p className="mt-2 text-sm text-zinc-400">Tài khoản hiện tại cần vai trò Quản trị viên.</p>
        </section>
      ) : (
        <>
          <section className="rounded-xl border border-zinc-800 bg-zinc-900/70 p-4">
            <div className="grid gap-3 xl:grid-cols-[minmax(160px,220px)_minmax(320px,1fr)_auto] xl:items-center">
              <div className="min-w-0">
                <p className="text-sm font-bold uppercase tracking-wide text-zinc-200">Tổng bài đăng: {total}</p>
                <p className="mt-1 flex items-center gap-3 text-xs text-zinc-500">
                  <span className="inline-flex items-center gap-1">
                    <IoEyeOutline aria-hidden /> {compactNumber(totals.views)} lượt xem
                  </span>
                  <span className="inline-flex items-center gap-1">
                    <IoHeartOutline aria-hidden /> {compactNumber(totals.likes)} lượt thích
                  </span>
                </p>
              </div>
              <input
              ...
              <table className="w-full min-w-[1080px] border-collapse text-left text-sm text-zinc-200">
                <thead>
                  <tr className="border-b border-zinc-800 text-xs text-zinc-500">
                    ...
                    <th className="px-3 py-3 text-center font-medium">Bình luận</th>
                ...
                <tbody>
                  {items.map((item) => {
                    const title = item.description || item.title || 'Bài đăng không có mô tả'
                    const hasThumb = item.thumbnailUrl && String(item.thumbnailUrl).trim()
                    return (
                      <tr key={item.publicId} className="border-b border-zinc-800/80">
                        <td className="py-3 pr-4">
                          <div className="flex items-center gap-3">
                            ...
                            <div className="min-w-0">
                              <p className="line-clamp-2 font-semibold text-zinc-100">{title}</p>
                              <p className="mt-0.5 truncate text-xs text-zinc-500">Mã #{item.publicId}</p>
                            </div>
                          </div>
                        </td>
                        <td className="px-3 py-3">
                          <p className="font-medium text-zinc-100">{item.authorDisplayName || 'Người dùng Vibely'}</p>
                          <p className="mt-0.5 text-xs text-zinc-500">@{item.authorUsername || 'unknown'}</p>
                        </td>
                        <td className="px-3 py-3">
                          <StatusBadge status={item.status} />
                        </td>
                        <td className="px-3 py-3 text-center tabular-nums">{compactNumber(item.viewCount)}</td>
                        <td className="px-3 py-3 text-center tabular-nums">{compactNumber(item.likeCount)}</td>
                        <td className="px-3 py-3 text-center tabular-nums">{compactNumber(item.commentCount)}</td>
                        <td className="px-3 py-3 text-center tabular-nums">{compactNumber(item.bookmarkCount)}</td>
                        <td className="whitespace-nowrap px-3 py-3 text-xs text-zinc-400">{formatDateTime(item.createdAt)}</td>
                        <td className="px-3 py-3">
