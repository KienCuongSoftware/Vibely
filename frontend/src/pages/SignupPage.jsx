...
import { IoAlertCircleOutline, IoArrowBack, IoChevronDown, IoClose, IoEyeOffOutline, IoEyeOutline } from 'react-icons/io5'
...
export function SignupPage() {
  ...
  const [status, setStatus] = useState('Chọn phương thức đăng ký để tiếp tục')
  const [loading, setLoading] = useState(false)
  const normalizedEmail = identifier.trim()
  const isEmailValid = /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(normalizedEmail)
  const showEmailError = emailTouched && !isEmailFocused && normalizedEmail.length > 0 && !isEmailValid
  const normalizedTikTokId = normalizeTikTokId(tikTokId)
  ...
  const canContinueToUsername =
    ...
    !sendingCode &&
    !loading
  const canSubmitUsername = otpVerified && normalizedTikTokId.length > 0 && usernameAvailable && !usernameChecking && !loading
  const passwordHasValidLength = password.length >= 8 && password.length <= 20
  const passwordHasRequiredCharacters = /[A-Za-z]/.test(password) && /\d/.test(password) && /[^A-Za-z0-9]/.test(password)
  const hasPasswordInput = password.length > 0
  const isPasswordValid = passwordHasValidLength && passwordHasRequiredCharacters
  ...
  const yearOptions = Array.from({ length: 2026 - 1900 + 1 }, (_, index) => String(2026 - index))
  const challengeShapes = [
    { id: 1, type: 'circle', label: 'Hình tròn A', className: 'left-8 top-10 h-12 w-12 bg-yellow-300' },
    { id: 2, type: 'triangle', label: 'Hình tam giác', className: 'left-28 top-8 h-14 w-14 bg-red-300 [clip-path:polygon(50%_0%,0%_100%,100%_100%)]' },
    { id: 3, type: 'hexagon', label: 'Lục giác', className: 'left-52 top-6 h-12 w-12 bg-sky-300 [clip-path:polygon(25%_0%,75%_0%,100%_50%,75%_100%,25%_100%,0%_50%)]' },
    { id: 4, type: 'circle', label: 'Hình tròn B', className: 'right-10 top-16 h-12 w-12 bg-yellow-300' },
    { id: 5, type: 'square', label: 'Hình vuông', className: 'left-40 bottom-10 h-12 w-12 bg-zinc-300' },
  ]
  ...
  useEffect(() => {
    ...
    if (!normalizedTikTokId) {
      return undefined
    }

    const timeoutId = setTimeout(() => {
      apiClient
        .checkUsername(normalizedTikTokId)
        .then((result) => {
  ...
  }, [normalizedTikTokId, view])

  const buildSuggestedUsername = () => {
    const fromEmail = normalizeTikTokId(normalizedEmail.split('@')[0] ?? '')
      .replace(/[^a-z0-9._]/g, '.')
      .replace(/\.+/g, '.')
      .replace(/^\.+|\.+$/g, '')
      .slice(0, 24)
  ...
  const handleChallengeConfirm = async () => {
    ...
    const selectedShapes = challengeShapes.filter((shape) => challengeSelection.includes(shape.id))
  ...
  const continueToUsernameStep = async (event) => {
    ...
    if (!birthMonth || !birthDay || !birthYear || !identifier.trim() || !password.trim() || verificationCode.trim().length !== 6) {
    ...
    const codeAlreadyVerified =
      verifiedCodeSnapshot === normalizedCode && verifiedEmailSnapshot === normalizedEmail
    if (!codeAlreadyVerified) {
      try {
        await apiClient.verifyCode({ email: normalizedEmail, code: normalizedCode })
  ...
  const submitRegisterWithUsername = async (event) => {
    ...
    if (!usernameAvailable) {
      const suggestionText = usernameSuggestion ? ` Gợi ý: @${usernameSuggestion}` : ''
    ...
    try {
      await register({
        username: normalizedTikTokId,
        displayName: normalizedTikTokId,
        email: normalizedEmail,
        password,
        bio: '',
      })
        ...
        <div className="min-h-0 flex-1 overflow-y-auto [scrollbar-gutter:stable] [scrollbar-width:thin] [scrollbar-color:#27272a_transparent] [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-zinc-800 [&::-webkit-scrollbar-track]:bg-zinc-950 [&::-webkit-scrollbar]:w-1.5">
          ...
          <>
            ...
            <div className="mx-auto w-full max-w-[380px] space-y-4 px-5 pb-7 text-sm">
              <h2 className="text-center text-[42px] font-bold leading-tight">Đăng ký Vibely</h2>
              <div className="mx-auto h-1 w-11/12 rounded-full bg-zinc-800" />

              <div className="space-y-3">
                <button
                  type="button"
                  className="flex w-full items-center gap-4 rounded-xl bg-zinc-800 px-4 py-3 text-left text-sm hover:bg-zinc-700"
                  onClick={() => {
                    setView('credentials')
                    setStatus('')
                  }}
              ...
              {status ? <p className="text-center text-xs text-zinc-400">{status}</p> : null}
            </div>
          </>
          ) : view === 'credentials' ? (
          <>
            ...
            <div className="mx-auto w-full max-w-[380px] space-y-3 px-5 pb-6 text-sm">
              <h2 className="text-center text-[42px] font-bold leading-tight">Đăng ký</h2>
              <p className="text-[13px] text-zinc-100">Vui lòng cho biết ngày sinh của bạn.</p>
              <form className="space-y-2.5" onSubmit={continueToUsernameStep}>
                ...
                <p className="text-[12px] text-zinc-500">Ngày sinh của bạn sẽ không được hiển thị công khai.</p>
                <div className="flex items-center justify-between">
                  <span className="text-[13px] font-semibold text-zinc-100">Email</span>
                  <button type="button" className="text-[12px] text-zinc-100 hover:text-zinc-300">
                ...
                <div className="relative">
                  <input
                    className={`h-10 w-full rounded bg-zinc-800 px-4 pr-10 text-[13px] ${
                      showEmailError ? 'border border-red-500 text-red-400 focus:outline-none' : ''
                    }`}
                ...
                {showEmailError ? <p className="text-[12px] text-red-500">Nhập địa chỉ email hợp lệ</p> : null}
                <div className="relative">
                  <input
                ...
                {isPasswordFocused ? (
                  <div className="space-y-0.5 text-[12px] text-zinc-300">
                    <p className="font-medium text-zinc-200">Mật khẩu của bạn phải gồm:</p>
                    <p
                      className={`pl-3 ${
                        !hasPasswordInput ? 'text-zinc-400' : passwordHasValidLength ? 'text-emerald-400' : 'text-red-400'
                      }`}
                    >
                    ...
                    >
                      {passwordHasRequiredCharacters ? '✓' : '·'} Các chữ cái, số và ký tự đặc biệt
                    </p>
                  </div>
                ) : null}
                <div className="flex">
                  ...
                  >
                    {sendingCode ? 'Đang gửi...' : resendSeconds > 0 ? `Gửi lại mã: ${resendSeconds}s` : 'Gửi mã'}
                  </button>
                </div>
                ...
                <button
                  className={`h-10 w-full rounded px-3 text-xl font-medium leading-none transition ${
                    canContinueToUsername ? 'bg-red-600 text-white hover:bg-red-500' : 'cursor-not-allowed bg-zinc-800 text-zinc-400'
                  }`}
                  type="submit"
                  disabled={!canContinueToUsername}
                >
                  {loading ? (
                    <span className="inline-flex items-center justify-center" aria-label="Đang xử lý">
          ...
          <>
            ...
            <div className="mx-auto w-full max-w-[380px] space-y-3 px-5 pb-6 text-sm">
              ...
              <form className="space-y-2.5" onSubmit={submitRegisterWithUsername}>
                <div className="relative">
                  ...
                  <input
                    ...
                    onChange={(event) => {
                      ...
                      setTikTokId(nextValue)
                      setUsernameChecking(normalizeTikTokId(nextValue).length > 0)
                      setUsernameAvailable(false)
                      setUsernameSuggestion('')
                      setUsernameMessage('')
                    }}
                ...
                {usernameSuggestion && !usernameAvailable ? (
                  <button
                    ...
                    onClick={() => {
                      setUsernameChecking(true)
                      setTikTokId(usernameSuggestion)
                    }}
                  >
                ...
                <button
                  className={`h-10 w-full rounded px-3 text-xl font-medium leading-none transition ${
                    canSubmitUsername ? 'bg-red-600 text-white hover:bg-red-500' : 'cursor-not-allowed bg-zinc-800 text-zinc-400'
                  }`}
                  type="submit"
              ...
              <button
                ...
                onClick={() => {
                  setUsernameChecking(true)
                  setTikTokId(buildSuggestedUsername())
                }}
              >
                Bỏ qua
              </button>
        ...
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/70 px-4">
          <div className="w-full max-w-[520px] rounded-2xl border border-zinc-700 bg-zinc-900 p-5 shadow-2xl">
            <div className="mb-4 flex items-center justify-between">
              <h3 className="text-xl font-semibold">Chọn 2 đối tượng có hình dạng giống nhau</h3>
              <button
                type="button"
                ...
                return (
                  <button
                    ...
                    className={`absolute rounded-full transition ${shape.className} ${
                      selectedIndex >= 0 ? 'ring-4 ring-red-500 ring-offset-2 ring-offset-zinc-100' : ''
                    }`}
            ...
            {challengeError ? <p className="mb-3 text-sm text-red-400">{challengeError}</p> : null}
            <button
              type="button"
              className={`h-12 w-full rounded-lg text-xl font-semibold text-white ${
                sendingCode ? 'cursor-not-allowed bg-red-400/60' : 'bg-red-500 hover:bg-red-400'
              }`}
