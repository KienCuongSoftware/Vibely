import { absoluteUrl, DEFAULT_DESCRIPTION, DEFAULT_TITLE, SITE_NAME } from './seoConfig.js'
...
export function profilePageJsonLd(profile, canonical) {
  ...
  const displayName = String(profile?.displayName ?? username ?? SITE_NAME).trim() || SITE_NAME
  return {
    '@context': 'https://schema.org',
    '@type': 'ProfilePage',
...
export function videoObjectJsonLd(video, canonical) {
  ...
  const description = String(video?.description ?? '').trim() || 'Xem video trên Vibely.'
  return {
    '@context': 'https://schema.org',
    '@type': 'VideoObject',
    name: title,
