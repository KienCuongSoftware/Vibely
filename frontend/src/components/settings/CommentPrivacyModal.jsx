...
export function commentAudienceLabel(value) {
  const found = COMMENT_AUDIENCE_OPTIONS.find((item) => item.value === value)
  return found?.label || 'Mọi người'
}
      ...
      >
        <div className="relative border-b border-zinc-800 px-5 pb-3 pt-4">
          <h2 id="comment-privacy-title" className="pr-10 text-base font-semibold text-zinc-100">
            Người có thể bình luận bài đăng của bạn
          </h2>
          <button
            type="button"
            onClick={onClose}
        ...
        <ul className="px-2 py-2" role="radiogroup" aria-label="Ai có thể bình luận">
