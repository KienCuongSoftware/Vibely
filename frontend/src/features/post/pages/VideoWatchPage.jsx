...
import { pickShareCaption } from "@/features/post/utils/shareCaption.js";
...
import { CommentInputAccessoryButtons } from "@/features/comment/components/CommentInputAccessory.jsx";
import { WatchSearchDropdown } from "@/features/search/components/WatchSearchDropdown.jsx";
import {
  IoArrowUp,
  IoBookmark,
...
} from "@/features/post/components/WatchVideoMoreMenu.jsx";
import { FeedReportedVideoOverlay } from "@/features/feed/components/FeedReportedVideoOverlay.jsx";
import { WatchShareStrip } from "@/features/post/components/WatchShareStrip.jsx";
...
import { sortQualityOptions } from "@/features/feed/utils/hlsQualityUtils.js";
    ...
    >
      <div className="pointer-events-auto flex min-h-7 w-full items-center gap-1 px-3 sm:gap-1.5 sm:px-4">
        <div
          ...
          onKeyDown={(e) => {
            ...
            onSeekFraction(
              Math.min(
                1,
                Math.max(0, (current + delta) / Math.max(duration, 1)),
              ),
            );
          }}
        >
          <div className="relative h-1 w-full rounded-full bg-white/30 transition-[height] duration-150 ease-out group-hover/watch-progress:h-1.5">
            ...
            <div
              className={`pointer-events-none absolute top-1/2 z-10 h-3.5 w-3.5 -translate-x-1/2 -translate-y-1/2 transition-opacity duration-200 ease-out ${
                isScrubbing
                  ? "opacity-100"
                  : "opacity-0 group-hover/watch-progress:opacity-100"
              }`}
              style={{ left: `${pct}%` }}
              aria-hidden
            >
        ...
        <div className="flex shrink-0 items-center gap-0.5">
          <TooltipHoverWrap tip={t('feedMenu.pip')} placement="top" hoverOnly>
            <button
              type="button"
              className={WATCH_BAR_ICON_BTN}
              aria-label={t('feedMenu.pip')}
              onClick={(e) => {
                e.stopPropagation();
                onTogglePip();
              }}
...
function WatchNowPlayingWave() {
  return (
    <span
      className="mb-1.5 flex h-4 items-end justify-center gap-[3px]"
      aria-hidden
    >
...
export function VideoWatchPage({ sidebarVariant = "creator" } = {}) {
  ...
  const [watchPlayback, setWatchPlayback] = useState({
    current: 0,
    duration: 0,
  });
  ...
  const [watchVideoQuality, setWatchVideoQuality] =
    usePersistedFeedVideoQuality();
  const [watchPlaybackSpeed, setWatchPlaybackSpeed] =
    usePersistedFeedPlaybackSpeed();
  ...
  const routeSlug = useMemo(
    () => normalizeUsernameKey(usernameParam),
    [usernameParam],
  );
  ...
  const isOwnCreatorProfile = useMemo(
    () =>
      Boolean(routeSlug) && normalizeUsernameKey(user?.username) === routeSlug,
  ...
  const resolvedVideo = useMemo(() => {
    ...
    if (video && normalizeVideoPublicId(video.publicId) === routeId)
      return video;
    ...
    if (isFromExplore) {
      return (
        exploreQueue.find(
          (row) => normalizeVideoPublicId(row?.publicId) === routeId,
        ) ?? null
      );
    }
    return null;
  ...
  const watchQualityMenuOptions = useMemo(
    () =>
      sortQualityOptions(
        watchQualityOptions.length ? watchQualityOptions : ["auto"],
      ),
    [watchQualityOptions],
  );
  ...
  const toggleWatchPlayback = useCallback(
    (e) => {
      e?.stopPropagation?.();
      if (watchScrubbingRef.current || watchMoreMenuOpen) return;
      // Left-click while menu is open: close menu (TikTok-like), don't also toggle.
      if (videoContextMenu) {
        setVideoContextMenu(null);
        return;
      }
  ...
  const handleWatchContextMenuDownload = useCallback(async () => {
    ...
    try {
      ...
      window.alert(
        error instanceof Error ? error.message : t('watch.loadFailed'),
      );
    } finally {
  ...
  const loadExploreChunk = useCallback(
    async (nextCursor) => {
      const slug =
        String(exploreContext?.slug ?? "").trim() &&
        exploreContext?.slug !== "all"
          ? exploreContext.slug
          : "all";
      return slug === "all"
  ...
  useEffect(() => {
    ...
    const seedItems = Array.isArray(exploreContext?.seedItems)
      ? exploreContext.seedItems
      : [];
    ...
    apiClient
      ...
      .then((page) => {
        ...
        setSuggestedVideos(
          items.filter(
            (row) =>
              normalizeVideoPublicId(row?.publicId) !== publicIdFromRoute,
          ),
        );
      })
      .catch(() => {
      ...
      try {
        const data =
          isOwnCreatorProfile && token
            ...
            : await apiClient.getVideosByUsername(slug, {
                page: 0,
                size: 48,
                token,
              });
  ...
  useEffect(() => {
    ...
    setCreatorQueue((prev) => {
      ...
      if (
        !id ||
        prev.some((row) => normalizeVideoPublicId(row?.publicId) === id)
      ) {
        return prev;
      }
    ...
    apiClient
      ...
      .catch((e) => {
        ...
        if (!optimistic) {
          setVideo(null);
          setLoadError(
            e instanceof Error ? e.message : t('watch.loadFailed'),
          );
        }
      })
      .finally(() => {
        if (!cancelled && loadGen === videoLoadGenRef.current)
          setLoading(false);
      });
    return () => {
      cancelled = true;
  ...
  useEffect(() => {
    ...
    apiClient
      ...
      .catch((e) => {
        if (!cancelled) {
          setCommentsError(
            e instanceof Error ? e.message : t('watch.loadCommentsFailed'),
          );
          setComments([]);
        }
      })
  ...
  const submitWatchComment = useCallback(async () => {
    ...
    try {
      ...
      setCommentPostError(
        e instanceof Error ? e.message : t('watch.sendCommentFailed'),
      );
    }
  }, [commentDraft, panelVideo, patchVideo, token]);
  const authorVibelyId =
  ...
  const seoVideoTitleSource =
    ...
    (isFromExplore ? t("watchPage.explorePageTitle") : t("watchPage.pageTitle"));
  ...
  useEffect(() => {
    if (watchSidebarTab !== "creator" && watchSidebarTab !== "suggested")
      return;
  ...
  const moveToCreatorVideoByOffset = useCallback(
    (offsetSteps) => {
      ...
      if (targetIndex < 0 || targetIndex >= creatorQueueRef.current.length)
        return;
    ...
    (target) => {
      ...
      const slug =
        ...
        normalizeUsernameKey(
          target?.authorUsername ?? panelVideo?.authorUsername,
        );
      if (!id || !slug) return;
  ...
  const handleRepostToggle = useCallback(() => {
    ...
    req
      .then(() => {
        if (next) {
          setRepostToast(t('watch.repostedToast'));
  ...
  return (
    <section
      className={`flex bg-black text-zinc-100 ${
        mobileLayout
          ? "min-h-dvh flex-col overflow-y-auto overscroll-y-auto"
          : "h-dvh min-h-0"
      }`}
    >
      <Seo
        ...
        jsonLd={
          panelVideo
            ? videoObjectJsonLd(panelVideo, absoluteUrl(seoVideoCanonical))
            : null
        }
      />
      <div
          ...
          >
            <div className="absolute inset-0">
                  ...
                  {isVideoFrameReady && activePlaybackUrl && !pipActive ? (
                    <>
                      ...
                      <WatchVideoMoreMenu
                        ...
                        videoPublicId={
                          panelVideo?.publicId ?? publicIdFromRoute
                        }
                        reportToken={token}
                        onReportRequireAuth={() => {
              ...
              {showVideoSpinner ? (
                <div
                  ...
                  aria-busy="true"
                  aria-label={
                    loading ? t('watch.loadingVideo') : t('watch.loadingPlayback')
                  }
                >
                  <WatchSpinner />
                </div>
            ...
            <div className="pointer-events-none absolute inset-x-0 top-0 z-30 flex h-16 items-center bg-linear-to-b from-black/70 via-black/30 to-transparent px-6 sm:h-18 sm:px-10">
              <div className="pointer-events-auto flex w-11 shrink-0 justify-start sm:w-12">
                <TooltipHoverWrap tip={t('common.close')} hoverOnly>
                  <button
                    type="button"
              ...
              <div
                className="pointer-events-none w-11 shrink-0 sm:w-12"
                aria-hidden
              />
            </div>
            ...
            {!activeVideo ? (
              ...
              <>
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-start gap-2">
                      <div className="min-w-0 flex-1">
                        <div className="flex flex-wrap items-baseline gap-x-2 gap-y-0.5">
                          ...
                          >
                            {String(
                              panelVideo.authorDisplayName ?? "",
                            ).trim() || t('common.creator')}
                          </Link>
                    ...
                    <div className="mt-2 flex items-center gap-1.5 text-xs text-zinc-400">
                      ...
                      <span className="truncate">
                        {t('watch.bgMusic', { id: authorVibelyId })}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex shrink-0 flex-nowrap items-center gap-x-4 overflow-visible border-b border-zinc-800 px-4 py-3">
                  <WatchActionTip tip={t('feed.like')}>
                    <button
                      type="button"
                  ...
                  <WatchActionTip tip={t('feed.comment')}>
                    <button
                      type="button"
                      className={`${ACTION_ROW} shrink-0`}
                  ...
                  <WatchActionTip tip={t('watch.saveTip')}>
                    <button
                      type="button"
                      className={`${ACTION_ROW} shrink-0`}
                      ...
                      title={
                        bookmarked
                          ? t('watch.savedTip')
                          : t('watch.saveToFavorites')
                      }
                      aria-label={bookmarked ? t('watch.unsave') : t('watch.save')}
                      onClick={() => {
                        if (!token || !isWatchableVideo(panelVideo)) {
                          if (!token) navigate("/login");
                          return;
                ...
                >
                  ...
                  <button
                    ...
                    onClick={() =>
                      setWatchSidebarTab(
                        useSuggestedSidebar ? "suggested" : "creator",
                      )
                    }
                  >
                    {useSuggestedSidebar
                      ? t('watch.youMayLike')
                      : t('watch.creatorVideos')}
                  </button>
                </div>
                  ...
                  {watchSidebarTab === "suggested" ? (
                    suggestedLoading ? (
                      <p className="py-10 text-center text-sm text-zinc-500">
                        {t('watch.loadingSuggestions')}
                      </p>
                    ) : suggestedVideos.length === 0 ? (
                      <p className="py-10 text-center text-sm text-zinc-500">
                        {t('watch.noSuggestions')}
                      </p>
                    ) : (
                    ...
                    creatorQueueLoading ? (
                      ...
                      <ul className="grid grid-cols-2 gap-2">
                        {creatorGridVideos.map((v) => {
                          const vid = videoPublicIdOf(v);
                          const playing =
                            vid != null && vid === publicIdFromRoute;
                          return (
                            <li key={vid ?? v.id}>
                              <WatchCreatorVideoTile
                                video={v}
