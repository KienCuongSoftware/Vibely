...
export const DM_AUDIENCE_OPTIONS = [
  { value: 'REQUEST', labelKey: 'settings.dm.request' },
  { value: 'OFF', labelKey: 'settings.dm.doNotReceive' },
]

export function dmAudienceLabel(value, t) {
  const found = DM_AUDIENCE_OPTIONS.find((item) => item.value === value)
  return t(found?.labelKey || 'settings.dm.request')
}

/**
 * Modal "Nhận tin nhắn trong" — Yêu cầu / Không nhận (kiểu TikTok).
 */
export function DmReceiveModal({
  open,
  title,
  value = 'REQUEST',
  helpText = '',
  ...
  const resolvedTitle = title || t('settings.dm.receiveIn')
  ...
  return (
    ...
    >
      <div
        role="dialog"
        aria-modal="true"
        aria-labelledby="dm-receive-title"
      ...
      >
        <div className="relative border-b border-zinc-800 px-5 pb-3 pt-4">
          <h2 id="dm-receive-title" className="pr-10 text-base font-semibold text-zinc-100">
            {resolvedTitle}
          </h2>
          <button
            type="button"
        ...
        <ul className="px-2 py-2" role="radiogroup" aria-label={resolvedTitle}>
          {DM_AUDIENCE_OPTIONS.map((option) => {
